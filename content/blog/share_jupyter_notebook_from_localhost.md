...
lastmod: 2026-08-24T14:12:25+05:30
draft: false
og_image: "images/jupyter/image0.webp"
tags: ["Jupyter", "Python", "http"]
schemahowto: ""
outputs:
  - HTML
  - AMP
---
...
{{< llm-context >}}To share a Jupyter Notebook from localhost with Pinggy - run `jupyter notebook` (starts on port 8888), then run `ssh -p 443 -R0:localhost:8888 -t free.pinggy.io "u:Host:localhost:8888" "u:Origin:http://localhost:8888"` to get a public HTTPS URL that routes back to it, and share that URL plus the notebook token with collaborators.{{< /llm-context >}}

Sharing a running process on your machine with someone else, without handing them a VPN login or a port-forwarding rule on your router, is a problem that comes up constantly for interactive tools like notebooks. {{< link href="https://jupyter.org/" >}}Jupyter Notebook{{< /link >}} allows users to create and share documents containing live code, equations, visualizations, and explanatory text. However, by default, Jupyter Notebook is limited to local access, requiring users to be on the same machine or network to collaborate effectively. This guide shows how you can share Jupyter Notebook with your teammates, collaborators, and friends.
...
3. Keep the last terminal running. In a separate terminal, start Pinggy tunnel:
   ...
   ```
   ssh -p 443 -R0:localhost:8888 -t free.pinggy.io "u:Host:localhost:8888" "u:Origin:http://localhost:8888"
   ```
...
{{< ssh_command >}}
"{\"cli\":{\"windows\":{\"ps\":\"./pinggy.exe -p 443 -R0:localhost:8888 -t free.pinggy.io \\\"a:Host:localhost:8888\\\" \\\"u:Origin:http://localhost:8888\\\"\",\"cmd\":\"./pinggy.exe -p 443 -R0:localhost:8888 -t free.pinggy.io \\\"a:Host:localhost:8888\\\" \\\"u:Origin:http://localhost:8888\\\"\"},\"linux\":{\"ps\":\"./pinggy -p 443 -R0:localhost:8888 -t free.pinggy.io \\\"a:Host:localhost:8888\\\" \\\"u:Origin:http://localhost:8888\\\"\",\"cmd\":\"./pinggy -p 443 -R0:localhost:8888 -t free.pinggy.io \\\"a:Host:localhost:8888\\\" \\\"u:Origin:http://localhost:8888\\\"\"}},\"ssh\":{\"windows\":{\"ps\":\"ssh -p 443 -R0:localhost:8888 -t free.pinggy.io \\\"a:Host:localhost:8888\\\" \\\"u:Origin:http://localhost:8888\\\"\",\"cmd\":\"ssh -p 443 -R0:localhost:8888 -t free.pinggy.io \\\"a:Host:localhost:8888\\\" \\\"u:Origin:http://localhost:8888\\\"\"},\"linux\":{\"ps\":\"ssh -p 443 -R0:localhost:8888 -t free.pinggy.io \\\"a:Host:localhost:8888\\\" \\\"u:Origin:http://localhost:8888\\\"\",\"cmd\":\"ssh -p 443 -R0:localhost:8888 -t free.pinggy.io \\\"a:Host:localhost:8888\\\" \\\"u:Origin:http://localhost:8888\\\"\"}}}"
{{</ ssh_command >}}
...
**Note on persistence:** the URL above is random and free tunnels expire after 60 minutes of use, which is fine for a quick pairing session but not for a notebook you're collaborating on over hours or days. Sign in at {{< link href="https://dashboard.pinggy.io/" >}}dashboard.pinggy.io{{< /link >}} and use a Pro token to get a persistent subdomain that survives tunnel restarts, so you don't have to redistribute a new link every time you reconnect.

The same command and header flags work for `jupyter lab` too, since JupyterLab runs on the same jupyter_server backend and enforces the same Host/Origin checks as classic Notebook.
