...
- **`import-article-zip`** - turn a zipped Google-Docs-exported article (one HTML file plus an `images/` folder, usually a guest or sponsored submission) into a blog post. Read [`.claude/skills/import-article-zip/SKILL.md`](./.claude/skills/import-article-zip/SKILL.md) whenever handed a `.zip` and asked to write a post from it. The hard rule is that every source link keeps its destination, anchor text, and paragraph; the skill covers unwrapping the `google.com/url?q=` redirects, the `cwebp` banner conversion, the base64 `schemahowto` block, the future-date trap that silently drops the page from the build, and the `categories`-overrides-the-eyebrow trap.
- **`pinggy`** - reference for Pinggy tunneling itself (SSH access methods, HTTP/TCP/UDP/TLS tunnel types, persistent subdomains/custom domains, auth, SDKs). Installed from the official feed at `https://pinggy.io/.well-known/skills/pinggy/` via `npx skills add https://pinggy.io`; scoped to this repo (not a global skill) since it's mainly useful when writing or fact-checking Pinggy how-to content here. Read [`.claude/skills/pinggy/SKILL.md`](./.claude/skills/pinggy/SKILL.md) when a blog post or page needs accurate tunnel commands/behavior.
